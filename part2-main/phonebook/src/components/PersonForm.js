...
const PersonForm = ({newName, setNewName, newNumber, setNewNumber, addPerson, focusName}) => {
    return (
        <form onSubmit={addPerson}>
            <div>
                FullName: <input value={newName} onChange={event => setNewName(event.target.value)} 
                    required autoComplete='off' ref={focusName}
                />
            </div>
            <div>
                MobileNumber: <input value={newNumber} onChange={event => setNewNumber(event.target.value)} 
